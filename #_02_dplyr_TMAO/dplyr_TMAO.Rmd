...
```{r}
library(readxl)
TMAO <- read_excel("~/Desktop/TMAO/#_02_dplyr_TMAO/TMAO_zhangshaowu_chinese.xlsx")
id_name_tmao <- read_excel("~/Desktop/TMAO/#_02_dplyr_TMAO/id_name_tmao.xlsx")
library(Tmisc)
library(tidyverse)
library(stringr)
TMAO <- dplyr::left_join(TMAO,id_name_tmao,by = c("ID号","姓名"))#combine the table 
TMAO$ECG <- factor(TMAO$ECG)
scr_z <- TMAO$肌酐
sex_z <- TMAO$性别
age_z <- TMAO$年龄
eGFR_Z <- c()
...
TMAO %>%
  ...
  mutate(CKD_stage =cut(eGFR, c(min(eGFR,na.rm=T),15,30,60,90,max(eGFR,na.rm=T)), right=FALSE,labels = (5:1)) ) %>%
...
### TMAO_1_4_5
```{r}
TMAO_1_4_5 <- read_excel("~/Desktop/TMAO/#_02_dplyr_TMAO/TMAO_zhangshaowu_chinese.xlsx")
id_name_tmao <- read_excel("~/Desktop/TMAO/#_02_dplyr_TMAO/id_name_tmao.xlsx")
...
### 散点图
```{r}
g <- ggplot(TMAO,aes(x = rank(eGFR),y = rank(TMAO)))
